...
8. **Mapping `--secondary-medium` (and other secondary derivatives) to `@crust`** — `--secondary-medium` is used for text labels in user profiles (`.about .secondary dt`) and other UI text. `@crust` is the darkest Catppuccin color (#11111b), making it unreadable on dark backgrounds. Use `@overlay0` / `@overlay2` / `@subtext1` for secondary-medium/high/very-high respectively — they should be lighter than `@base`, not darker.
